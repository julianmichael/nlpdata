...
object PTB3Path {
  ...
  private[this] val WSJPathRegex =
    """([0-9]{2})/WSJ_[0-9]{2}([0-9]{2})\.MRG""".r
  ...
  def fromPTBPath(path: nlpdata.datasets.ptb.PTBPath) = path.suffix match {
    case WSJPathRegex(IntMatch(section), IntMatch(number)) =>
      Some(WSJPath(section, number))
    case _ => None
  }
  ...
  def toPTBPath(path: PTB3Path) = path match {
    case WSJPath(section, number) =>
      Some(
        nlpdata.datasets.ptb
          .PTBPath(f"$section%02d/WSJ_$section%02d$number%02d.MRG")
      )
    case _ => None
  }
}
...
object PTB3SentencePath {
  ...
  def fromPTBSentencePath(sentencePath: nlpdata.datasets.ptb.PTBSentencePath) =
    sentencePath match {
  ...
  def toPTBSentencePath(sentencePath: PTB3SentencePath) = sentencePath match {
    case PTB3SentencePath(path, sentenceNum) =>
      PTB3Path
        .toPTBPath(path)
        .map(nlpdata.datasets.ptb.PTBSentencePath(_, sentenceNum))
  }
}
...
case class PTB3SentencePath(filepath: PTB3Path, sentenceNum: Int)

case class PTB3File(path: PTB3Path, sentences: Vector[PTB3Sentence])

case class PTB3Sentence(path: PTB3SentencePath, words: Vector[Word], syntaxTree: SyntaxTree)
