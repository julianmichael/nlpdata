...
case class PropBankFile(path: PropBankPath, sentences: Vector[PropBankSentence])
...
case class PropBankSentence(
  ...
  val (words, predicateArgumentStructures) = {
    val wordMapping = wordsOriginal
      .foldRight((List.empty[List[Word]], false)) {
        case (w, (ws, hasHyphen)) =>
          ws match {
            case prevGroup :: rest if hasHyphen =>
              ((w :: prevGroup) :: rest, false)
            case prevGroup :: rest if w.pos == "HYPH" =>
              ((w :: prevGroup) :: rest, true)
      ...
      .toMap
    val newWords =
      wordsOriginal.map(wordMapping).toSet.toList.sortBy((w: Word) => w.index)
