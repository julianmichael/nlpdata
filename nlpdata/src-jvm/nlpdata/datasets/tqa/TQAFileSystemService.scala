...
object TQAFileSystemService {
  ...
  object JsonParsing {
    ...
    def figuresFromJson(figuresJson: Json): List[TQAFigure] = {
      figuresJson.array.get.iterator
        .map(
          figureJson =>
            TQAFigure(
    ...
    def adjunctTopicFromSectionNameAndJson(
      sectionName: String,
      adjTopicJson: Json
    ): TQAAdjunctTopic = {
    ...
    def adjunctTopicsAndVocabularyFromJson(
      adjTopicsJson: Json
    ): (List[TQAAdjunctTopic], List[TQAVocabularyItem]) = {
      val vocabObject = adjTopicsJson.field("Vocabulary").get
      val vocabulary = vocabObject.objectFields.get.iterator
        .map(word => TQAVocabularyItem(word, vocabObject.field(word).get.string.get.toString))
        .toList
      val adjunctTopics = adjTopicsJson.objectFields.get.iterator
        .filterNot(_ == "Vocabulary")
        .map(
          sectionName =>
            adjunctTopicFromSectionNameAndJson(sectionName, adjTopicsJson.field(sectionName).get)
        )
        .toList
    ...
    def answerChoicesFromJson(answerChoicesJson: Json): List[TQAAnswerChoice] = {
      answerChoicesJson.objectFields.get.iterator
        .map(
          answerLabel =>
            TQAAnswerChoice(
              answerLabel,
              labeledTextFromJson(answerChoicesJson.field(answerLabel).get)
          )
        )
        .toList
    }

    def nonDiagramQuestionFromIdAndJson(
      globalId: String,
      nonDiagramQuestionJson: Json
    ): TQANonDiagramQuestion = {
      TQANonDiagramQuestion(
        globalId = globalId,
        questionType = nonDiagramQuestionJson.field("questionType").get.string.get.toString,
        questionSubType = nonDiagramQuestionJson
          .field("questionSubType")
          .get
          .string
          .get
          .toString,
        text = textFromJson(nonDiagramQuestionJson.field("beingAsked").get)
          .withIdStructural(
            nonDiagramQuestionJson
              .field("idStructural")
              .get
              .string
              .get
    ...
    def nonDiagramQuestionsFromJson(
      nonDiagramQuestionsJson: Json
    ): Map[String, TQANonDiagramQuestion] = {
      nonDiagramQuestionsJson.objectFields.get.iterator.map { globalId =>
        globalId -> nonDiagramQuestionFromIdAndJson(
          globalId,
          nonDiagramQuestionsJson.field(globalId).get
        )
      }.toMap
    }
    ...
    def annotationsFromJson(annotationsJson: Json): List[TQADiagramAnnotation] = {
      annotationsJson.array.get.iterator.map { obj =>
        ...
        TQADiagramAnnotation(text = obj.field("text").get.string.get.toString, location = location)
      }.toList
    }

    def diagramsFromDiagramsAndAnnotationsJson(
      diagramsJson: Json,
      annotationsJson: Json
    ): Map[String, TQADiagram] = {
    ...
    def lessonFromJson(lessonJson: Json): TQALesson = {
      ...
      TQALesson(
        ...
        diagramQuestions =
          diagramQuestionsFromJson(lessonJson.field("questions").get.field("diagramQuestions").get),
        nonDiagramQuestions = nonDiagramQuestionsFromJson(
          lessonJson.field("questions").get.field("nonDiagramQuestions").get
        ),
        diagrams = diagrams
      )
    }
